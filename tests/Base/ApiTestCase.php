...
abstract class ApiTestCase extends BaseTestCase
{
    ...
    protected function assertJsonStructure(array $structure, ?string $path = null): void
    {
        $this->response->assertJsonStructure($structure, $path);
    }

    protected function assertJsonPath(string $path, mixed $expected): void
    {
        $this->response->assertJsonPath($path, $expected);
    }

    protected function assertJsonFragment(array $fragment): void
    {
        $this->response->assertJsonFragment($fragment);
    }

    protected function assertJsonMissing(array $data): void
    {
        $this->response->assertJsonMissing($data);
    }

    protected function assertJsonMissingExact(array $data): void
    {
        $this->response->assertJsonMissingExact($data);
    }

    protected function assertJsonValidationErrors(array $errors, ?string $responseKey = 'errors'): void
    {
        $this->response->assertJsonValidationErrors($errors, $responseKey);
    }

    protected function assertJsonMissingValidationErrors(?string $key = null): void
    {
        $this->response->assertJsonMissingValidationErrors($key);
    }

    protected function withJsonHeaders(): self
    {
    ...
    protected function assertApiUnprocessableEntity(): void
    {
        $this->response->assertStatus(422);
    }

    protected function assertApiError(int $status = 500): void
    {
