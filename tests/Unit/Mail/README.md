# Mail classes -- Automated Unit Tests
...
Unit tests for **Mail classes** -- the code that builds and sends emails / notifications. These tests check each piece on its own (without the database or other parts of the system) to make sure it works correctly.
...
# A single test
php artisan test tests/Unit/Mail --filter=TestName
```
