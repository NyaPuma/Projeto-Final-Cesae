...
Unit tests for **Events** -- announcements that one part of the system fires so others can react. These tests check each piece on its own (without the database or other parts of the system) to make sure it works correctly.
...
# A single test
php artisan test tests/Unit/Events --filter=TestName
```
