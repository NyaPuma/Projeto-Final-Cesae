# Observers -- Automated Unit Tests
...
Unit tests for **Observers** -- the code that reacts to model lifecycle events (created, updated, deleted). These tests check each piece on its own (without the database or other parts of the system) to make sure it works correctly.
...
# A single test
php artisan test tests/Unit/Observers --filter=TestName
```
