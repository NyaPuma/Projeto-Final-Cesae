# API Resources -- Automated Unit Tests
...
Unit tests for **API Resources** -- the code that shapes data before returning it in API responses. These tests check each piece on its own (without the database or other parts of the system) to make sure it works correctly.
...
php artisan test tests/Unit/Resources

# A single test
php artisan test tests/Unit/Resources --filter=TestName
```
