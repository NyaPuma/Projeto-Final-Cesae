...
Unit tests for **Services** -- the 'supervisors' that coordinate complex multi-step operations. These tests check each piece on its own (without the database or other parts of the system) to make sure it works correctly.
...
# A single test
php artisan test tests/Unit/Services --filter=TestName
```
