# Console Commands -- Automated Unit Tests
...
Unit tests for **Console Commands** -- the text-based admin commands developers run. These tests check each piece on its own (without the database or other parts of the system) to make sure it works correctly.
...
# A single test
php artisan test tests/Unit/Console --filter=TestName
```
