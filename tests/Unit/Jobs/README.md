...
Unit tests for **Jobs** -- background tasks that run asynchronously (e.g., sending emails). These tests check each piece on its own (without the database or other parts of the system) to make sure it works correctly.
...
# A single test
php artisan test tests/Unit/Jobs --filter=TestName
```
