...
Unit tests for **Traits** -- reusable code snippets shared between multiple classes. These tests check each piece on its own (without the database or other parts of the system) to make sure it works correctly.
...
# A single test
php artisan test tests/Unit/Traits --filter=TestName
```
