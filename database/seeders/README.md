...
| `UsersSeeder.php` | **Users** — a handful of default demo accounts (admin/technician/user with realistic passwords) plus ~100 synthetic users with a realistic mix of roles and active/inactive states. Refuses to run in production. |
| `RoomsSeeder.php` | **Rooms** — a few realistic rooms (assembly line, I&D lab, warehouse…) plus the full room catalogue and ~100 synthetic rooms with a realistic building/floor mixture and ~15% set inactive. |
| `EquipmentCategoriesSeeder.php` | **Equipment categories** — the four main real ones (Robotics, Automation, Infrastructure, Logistics) plus ~100 synthetic categories. |
| `EquipmentsSeeder.php` | **Equipment** — realistic machines (robotic arms, presses, servers, forklifts…) plus the full catalogue and ~100 synthetic items, with a realistic status mix (~80% operational, ~10% maintenance, ~5% broken, ~5% withdrawn). |
...
| `TicketsSeeder.php` | **Tickets** — ~1,500 realistic maintenance tickets with varied priorities and a realistic status mix (~85% final states, ~15% active). Refuses to run in production. |
...
| `NotificationSeeder.php` | **Notifications** — ~600 sample notifications for users, with realistic types, priorities, and read/unread states. Only seeds if notifications are empty. |
