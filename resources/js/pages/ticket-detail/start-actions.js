...
export function bindRepairStartActions(fetchTicket) {
    document.getElementById('btnStartRepair')?.addEventListener('click', async () => {
        ...
        if (startButton) {
            ...
            startButton.innerHTML = '<span class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></span> Checking...';
        }
        ...
        try {
            ...
            if (startButton && forceButton?.classList.contains('hidden')) {
                ...
                startButton.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.348a1.125 1.125 0 010 1.971l-11.54 6.347a1.125 1.125 0 01-1.667-.985V5.653z"></path></svg> Start Intervention';
            }
        }
    });
    ...
    document.getElementById('btnStartRepairForce')?.addEventListener('click', async () => {
        ...
        if (forceButton) {
            ...
            forceButton.innerHTML = '<span class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></span> Starting...';
        }
