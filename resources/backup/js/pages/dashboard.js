...
async function loadMetrics() {
    ...
    if (userRole !== 'admin') {
        panel.innerHTML = `
            <div class="rounded-xl border border-dashed border-(--border) bg-(--surface-2) p-5 col-span-full text-center">
                <p class="text-xs text-(--text-soft)">Métricas disponíveis apenas para Administrador.</p>
            </div>
        `;
        loadRecentTickets();
        return;
    ...
    try {
        ...
        panel.innerHTML = `
            <div class="rounded-2xl border border-(--border) bg-(--surface) p-5 shadow-sm animate-[fadeIn_0.3s_ease-out]">
                <p class="text-[10px] font-bold uppercase tracking-wider text-(--text-soft)">Tempo Médio Resolução</p>
                <p class="mt-2 text-2xl font-black text-(--text)">${data.average_resolution_human ?? '0h 0m'}</p>
                <p class="mt-0.5 text-xs font-semibold text-(--text-soft)">${data.average_resolution_minutes ?? 0} min</p>
            </div>
            <div class="rounded-2xl border border-(--border) bg-(--surface) p-5 shadow-sm animate-[fadeIn_0.3s_ease-out]">
                <p class="text-[10px] font-bold uppercase tracking-wider text-(--text-soft)">Tempo Médio Espera</p>
                <p class="mt-2 text-2xl font-black text-(--text)">${data.average_waiting_human ?? '0h 0m'}</p>
                <p class="mt-0.5 text-xs font-semibold text-(--text-soft)">${data.average_waiting_minutes ?? 0} min</p>
            </div>
            <div class="rounded-2xl border border-(--border) bg-(--surface) p-5 shadow-sm animate-[fadeIn_0.3s_ease-out] flex flex-col justify-between">
                <p class="text-[10px] font-bold uppercase tracking-wider text-(--text-soft)">Tickets em Aberto</p>
                <p class="mt-2 text-3xl font-black text-amber-500">${data.open_tickets ?? 0}</p>
            </div>
            <div class="rounded-2xl border border-(--border) bg-(--surface) p-5 shadow-sm animate-[fadeIn_0.3s_ease-out] flex flex-col justify-between">
                <p class="text-[10px] font-bold uppercase tracking-wider text-(--text-soft)">Tickets Fechados</p>
...
async function loadRecentTickets() {
    ...
    if (tickets.length === 0) {
        tableContainer.innerHTML = `<p class="text-xs text-(--text-soft) py-2">Nenhuma ocorrência recente registada.</p>`;
        return;
    }
