...
>
    ...
    <div class="mb-6 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="sm:col-span-2 lg:col-span-3">
                <label for="filter_q" class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-[var(--text-soft)]">{{ __('common.Pesquisa Geral') }}</label>
            ...
            <div>
                <label for="filter_event" class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-[var(--text-soft)]">{{ __('common.Ação / Evento') }}</label>
        ...
        <div class="mt-4 pt-4 border-t border-[var(--border)] flex flex-col-reverse items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:gap-2">
            <div class="flex items-center gap-2">
                <button id="btnSearch" class="inline-flex flex-1 sm:flex-none items-center justify-center px-4 py-2 bg-primary text-[var(--on-primary)] text-xs font-bold rounded-xl shadow-sm hover:bg-primary-hover transition-all cursor-pointer min-h-[36px]">
                    {{ __('ui.Pesquisar') }}
                </button>
    ...
    <div class="ui-listing-table w-full overflow-hidden bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-sm">
        <div class="overflow-x-auto">
            <table class="w-full divide-y divide-[var(--border)] text-left text-sm">
