...
@component('ui.partials.page-card', [
    'title' => __('Os Meus Tickets'),
    'subtitle' => __('Consulte e gira as ocorrências atribuídas a si ou submetidas por si.'),
    ...
    <div class="mb-6 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm animate-[fadeIn_0.2s_ease-out]">
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            ...
            <div class="sm:col-span-2 lg:col-span-3 xl:col-span-4">
                ...
                <div class="relative">
                    <input id="filter_q" placeholder="{{ __('Pesquisar nos meus tickets...') }}"
    ...
    <div class="w-full overflow-hidden bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-sm" role="region" aria-live="polite" aria-label="{{ __('Lista dos meus tickets') }}">
...
async function loadTickets(page = 1) {
    ...
    const endpoint = '/api/tickets/my';
    const url = `${endpoint}?${params.toString()}`;

    const tbody = document.getElementById('ticketsBody');
    tbody.innerHTML = `<tr><td colspan="7" class="px-5 py-12 text-center text-xs text-[var(--text-soft)]">${"{{ __('A atualizar dados...') }}"}</td></tr>`;

    try {
        // Adicionado credentials: 'same-origin' para garantir que os cookies da sessão transitam para a API
        ...
        if (!res.ok) {
            ...
            showFeedback(errData.message || "{{ __('Não foi possível carregar os seus tickets de momento.') }}", true);
            return;
        }

        const data = await res.json().catch(() => ({}));

        // 🔧 CORREÇÃO DA ESTRUTURA DO LARAVEL PAGINATE:
        // Se res.json() é o próprio paginator: data.data são os itens, data.total é o total, data.last_page é a última página.
        const tickets = data.data ?? data.tickets?.data ?? data.tickets ?? (Array.isArray(data) ? data : []);
        const total   = data.total ?? data.tickets?.meta?.total ?? tickets.length;
        const lastPage = data.last_page ?? data.tickets?.meta?.last_page ?? 1;
        const currPage = data.current_page ?? data.tickets?.meta?.current_page ?? page;

        document.getElementById('resultsCount').textContent = total > 0 ? `${total} ${"{{ __('resultado(s) encontrado(s)') }}"}` : "{{ __('Sem resultados') }}";

        if (!tickets.length) {
            tbody.innerHTML = `<tr><td colspan="7" class="px-5 py-12 text-center text-xs text-[var(--text-soft)]"><div class="mx-auto max-w-sm rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface-2)] p-5">${"{{ __('Não tem nenhum ticket registado com os filtros aplicados.') }}"}</div></td></tr>`;
        ...
        showFeedback("{{ __('Erro ao carregar os seus tickets.') }} " + err.message, true);
    }
}
