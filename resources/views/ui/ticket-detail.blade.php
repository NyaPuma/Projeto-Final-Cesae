    ...
    <div class="grid gap-4 xl:grid-cols-[1.2fr_0.8fr] items-start">
        ...
        <div class="space-y-4">
            ...
            <div class="rounded-2xl border border-(--border) bg-(--surface) p-4 shadow-sm space-y-3">
                <h3 class="text-xs font-bold uppercase tracking-wider text-(--text) border-b border-(--border) pb-2">{{ __('common.Histórico & Comentários') }}</h3>
                <div id="commentsSection"
                    ...
                    <p class="italic py-1 text-center text-[11px]">{{ __('ui.A carregar histórico...') }}</p>
                </div>
                <form id="commentForm" class="flex gap-2 items-center pt-2 border-t border-(--border)">
                    <input id="commentText" type="text" required class="flex-1 rounded-xl border border-(--border) bg-(--surface-2) px-3 py-2 text-xs text-(--text) outline-none focus:border-primary transition-all" placeholder="{{ __('messages.Escreva uma mensagem...') }}">
            ...
            <div class="rounded-2xl border border-(--border) bg-(--surface) p-4 shadow-sm space-y-3">
                <div class="flex items-center justify-between border-b border-(--border) pb-2">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-(--text)">{{ __('common.Evidências Fotográficas') }}</h3>
                    <span class="text-[9px] font-bold text-(--text-soft) uppercase tracking-wider">{{ __('ticket_media.Anexos') }}</span>
                </div>
                ...
                <div id="photosSection"
                    ...
                    <p class="italic text-[11px]">{{ __('common.Nenhuma evidência carregada.') }}</p>
                </div>
            </div>
        ...
        <div class="space-y-4">
            ...
            <div id="budgetApprovalCard" class="hidden rounded-2xl border border-amber-500/40 bg-(--surface) p-4 shadow-sm space-y-3">
                <div class="flex items-center justify-between border-b border-(--border) pb-2">
                    <span class="px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-amber-500 bg-amber-500/10 border border-amber-500/20 rounded-lg">{{ __('common.Aprovação Orçamental') }}</span>
                    <span id="budgetEstimatedCost" class="text-xs font-black text-amber-400">(0)</span>
                </div>
                <div>
                    <p class="text-[11px] text-(--text-soft) leading-tight">{{ __('common.Limiar de aprovação:') }} <span id="budgetThresholdDisplay">(0)</span></p>
                    <p class="text-[11px] text-(--text-soft) leading-tight">{{ __('common.Técnico:') }} <span id="budgetTechnicianName">—</span></p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <button type="button" id="btnApproveBudget" class="py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-sm transition cursor-pointer">{{ __('common.Validar Orçamento') }}</button>
                    <button type="button" id="btnRejectBudget" class="py-2 bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold rounded-xl shadow-sm transition cursor-pointer">{{ __('common.Não Validar') }}</button>
                </div>
            </div>

            <div class="rounded-2xl border border-(--border) bg-(--surface) p-4 shadow-sm space-y-3">
                <div class="flex items-center justify-between border-b border-(--border) pb-2">
                    <span class="px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-primary bg-primary/10 border border-primary/20 rounded-lg">{{ __('dashboard.Painel do Admin') }}</span>
                    <span id="adminTicketId" class="text-xs font-mono font-bold text-(--text-soft)">#{{ $ticketId ?? $ticket->id ?? '' }}</span>
                </div>
                <div>
                    <h3 class="text-xs font-bold text-(--text)">{{ __('common.Atribuição de Técnico') }}</h3>
                    <p class="text-[11px] text-(--text-soft) mt-0.5 leading-tight">{{ __('common.Defina manualmente o responsável ou solicite à IA para triagem automática.') }}</p>
                </div>
                <div class="space-y-2 pt-1">
                    <div>
                        <label class="block text-[9px] font-bold uppercase tracking-wider text-(--text-soft) mb-1">{{ __('common.ID do Técnico (Manual)') }}</label>
                    ...
                    <div class="space-y-2 pt-1">
                        ...
                        <button id="btnAssignAuto" type="button" class="w-full py-2 bg-primary hover:bg-primary-hover text-white text-xs font-extrabold rounded-xl shadow-md transition cursor-pointer flex items-center justify-center gap-1.5">{{ __('common.Atribuição Automática (IA)') }}</button>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div id="techStartCard" class="hidden rounded-2xl border border-primary/30 bg-(--surface) p-4 shadow-sm space-y-3">
                <div class="flex items-center justify-between border-b border-(--border) pb-2">
                    <span class="text-[9px] font-bold uppercase tracking-widest text-primary bg-primary/10 px-2 py-0.5 rounded-lg">{{ __('common.Operacional') }}</span>
                    <span class="text-xs font-bold text-amber-500">{{ __('common.Livre') }}</span>
                </div>
                <div>
                    <h3 class="text-xs font-bold text-(--text)">{{ __('tickets.Assumir Ocorrência') }}</h3>
                    <p class="text-[11px] text-(--text-soft) mt-0.5 leading-tight">{{ __('tickets.Este ticket encontra-se livre. Caso tenha disponibilidade, assuma a reparação.') }}</p>
                </div>
                <div class="space-y-2">
                    <button type="button" id="btnStartRepair" class="w-full inline-flex items-center justify-center rounded-xl py-2.5 text-xs font-black uppercase tracking-wider bg-primary text-white hover:opacity-90 shadow-md shadow-primary/20 transition cursor-pointer">
                    ...
                    <button type="button" id="btnStartRepairForce" class="hidden w-full py-2 bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold rounded-xl shadow-sm transition cursor-pointer flex items-center justify-center gap-1.5">
                        <span>⚠️</span> {{ __('common.Forçar Início (ignorar prioritários)') }}
                    </button>
                </div>
            </div>
            ...
            <div id="techBudgetSubmitCard" class="hidden rounded-2xl border border-primary/30 bg-(--surface) p-4 shadow-sm space-y-3">
                <div class="flex items-center justify-between border-b border-(--border) pb-2">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-(--text)">{{ __('common.Avaliação Orçamental Detalhada') }}</h3>
                </div>
                <p class="text-[11px] text-(--text-soft) leading-tight">{{ __('tickets.Introduza o orçamento estimado. Se o total exceder 50€, o ticket aguardará autorização da Administração.') }}</p>
                <div class="space-y-3 pt-1">
                    <div>
                        <label class="block text-[9px] font-bold uppercase tracking-wider text-(--text-soft) mb-1.5">{{ __('common.Itens do Orçamento') }}</label>
                        <div id="budgetItemsContainer" class="space-y-1.5 mb-2"></div>
                        <button type="button" id="btnAddBudgetItem" class="inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-bold text-primary bg-primary/10 border border-primary/30 rounded-lg hover:bg-primary/20 transition cursor-pointer">
                            + {{ __('ui.ADICIONAR ITEM') }}
                        </button>
                    </div>
                    <div class="p-2.5 bg-(--surface-2) border border-(--border) rounded-xl flex items-center justify-between">
                        <span class="text-[10px] font-bold uppercase text-(--text-soft)">{{ __('common.Total Estimado') }}</span>
                    ...
                    <button type="button" id="btnSubmitEstimatedBudget" class="w-full py-2.5 px-3 bg-primary hover:bg-primary-hover text-white text-xs font-bold rounded-xl shadow-sm transition cursor-pointer">
            ...
            <div id="techBlockedCard" class="hidden rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 shadow-sm space-y-2 text-center">
                <div class="text-xs font-bold text-amber-500">{{ __('ui.Aguardar Validação Orçamental') }}</div>
                <p class="text-[11px] text-(--text-soft) leading-tight">{{ __('tickets.O orçamento excede o limiar. O ticket aguarda aprovação da Administração.') }}</p>
            </div>

            <div id="techApprovedCard" class="hidden rounded-2xl border border-emerald-500/30 bg-(--surface) p-4 shadow-sm space-y-2">
                <div class="flex items-center justify-between border-b border-(--border) pb-2">
                    <span class="text-[9px] font-bold uppercase tracking-widest text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-lg">{{ __('common.Autorizado') }}</span>
                    <span class="text-xs font-bold text-emerald-400">{{ __('common.Aprovado') }}</span>
                </div>
                <h3 class="text-xs font-bold text-(--text)">{{ __('common.Orçamento Aprovado') }}</h3>
                <p class="text-[11px] text-(--text-soft) leading-tight">{{ __('common.O orçamento foi aprovado. Pode iniciar a reparação.') }}</p>
            </div>

            <div id="techRejectedCard" class="hidden rounded-2xl border border-rose-500/30 bg-rose-500/5 p-4 shadow-sm space-y-2">
                <div class="flex items-center justify-between border-b border-(--border) pb-2">
                    <span class="text-[9px] font-bold uppercase tracking-widest text-rose-500 bg-rose-500/10 px-2 py-0.5 rounded-lg">{{ __('common.Recusado') }}</span>
                </div>
            ...
            <div id="techCompletionCard" class="hidden rounded-2xl border border-emerald-500/30 bg-(--surface) p-4 shadow-sm space-y-3">
                <div class="flex items-center justify-between border-b border-(--border) pb-2">
                    <span class="text-[9px] font-bold uppercase tracking-widest text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-lg">{{ __('common.A minha intervenção') }}</span>
                    <span class="text-xs font-bold text-emerald-400">{{ __('common.Autorizado') }}</span>
                </div>
                <p class="text-[11px] text-(--text-soft) leading-tight">{{ __('common.Intervenção autorizada. Conclua os trabalhos e registe os dados finais.') }}</p>
                <div class="space-y-2.5 pt-1">
                    <div>
                        <label class="block text-[9px] font-bold uppercase tracking-wider text-(--text-soft) mb-1">{{ __('common.Custo Final (€)') }}</label>
                    ...
                    <div>
                        <label class="block text-[9px] font-bold uppercase tracking-wider text-(--text-soft) mb-1">{{ __('common.Relatório Técnico') }}</label>
    ...
    <div id="priorityWarningModal" class="hidden fixed inset-0 z-[9999] items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div class="relative w-full max-w-md rounded-2xl border border-(--border) bg-(--surface) p-6 shadow-2xl space-y-4">
            <div class="flex items-center justify-between">
                <span class="text-xs font-bold uppercase tracking-wider text-amber-500 bg-amber-500/10 px-2 py-0.5 rounded-lg">{{ __('common.Prioridades') }}</span>
                <span class="text-lg font-bold text-amber-400">⚠️</span>
            </div>
            <div>
                <h3 class="text-sm font-black text-(--text)">{{ __('tickets.Atenção: Tickets Prioritários') }}</h3>
            ...
            <div class="grid grid-cols-2 gap-2 pt-2">
                <button type="button" id="btnViewUrgentTickets" class="py-2 px-3 bg-primary text-white text-xs font-bold rounded-xl hover:opacity-90 transition cursor-pointer">{{ __('tickets.Ver Tickets Urgentes') }}</button>
                <button type="button" id="btnForceStartTicket" class="py-2 px-3 bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold rounded-xl transition cursor-pointer">{{ __('common.Prosseguir Mesmo Assim') }}</button>
            </div>
        </div>
    </div>
