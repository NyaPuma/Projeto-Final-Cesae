...
async function loadUsers(page = 1) {
    ...
    try {
        ...
        users.forEach(user => {
            ...
            // Construir a URL do avatar com fallback para iniciais
            ...
            tr.innerHTML = `
                ...
                <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full overflow-hidden border border-[var(--border)] bg-[var(--surface-2)] flex-shrink-0 shadow-sm flex items-center justify-center">
                            <img src="${avatarSrc}" 
                                 alt="${user.name || ''}" 
                                 class="w-full h-full object-cover" 
                                 onerror="this.onerror=null; this.src='${fallbackAvatar}';">
                        </div>
                        <span class="font-bold text-[var(--text)]">${user.name || ''}</span>
                    </div>
                </td>
                ...
                <td class="px-6 py-4 text-right">
                    <a href="/ui/users/${user.id}/edit" class="inline-flex items-center justify-center px-3.5 py-1.5 bg-[var(--surface)] hover:bg-[var(--surface-2)] text-[11px] font-semibold text-[var(--text)] border border-[var(--border)] rounded-lg shadow-sm transition-all min-h-[28px]">${"{{ __('Editar') }}"}</a>
                </td>
            `;
