    ...
    <div class="mb-6 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
        ...
        <div class="mt-4 pt-4 border-t border-[var(--border)] flex flex-wrap items-center justify-between gap-3">
            <div class="flex items-center gap-2">
                <button id="btnSearch" onclick="loadRooms(1)" class="inline-flex items-center justify-center px-5 py-2 text-xs font-bold text-white bg-orange-500 hover:bg-orange-600 rounded-xl shadow-sm transition-all cursor-pointer min-h-[36px]">
                    {{ __('Pesquisar') }}
                </button>
                <button id="btnClear" onclick="clearFilters()" class="inline-flex items-center justify-center px-5 py-2 text-xs font-semibold text-orange-500 bg-transparent border border-orange-500/40 hover:bg-orange-500/10 rounded-xl transition-all cursor-pointer min-h-[36px]">
                    {{ __('Limpar filtros') }}
                </button>
            </div>
            <div>
...
async function loadRooms(page = 1) {
    ...
    if (tbody) tbody.innerHTML = '<tr><td colspan="4" class="px-5 py-12 text-center text-xs text-[var(--text-soft)]">A atualizar dados...</td></tr>';

    try {
        const res = await fetchWithFallback(`/rooms?${params.toString()}`, { headers: authHeader() });

        if (!res || !res.ok) {
            document.getElementById('resultsCount').textContent = "Erro ao carregar dados";
            if (tbody) tbody.innerHTML = '<tr><td colspan="4" class="px-5 py-12 text-center text-xs text-red-400">Não foi possível ligar ao servidor de salas.</td></tr>';
            return;
        }
        ...
        if (!rooms.length) {
            tbody.innerHTML = '<tr><td colspan="4" class="px-5 py-12 text-center text-xs text-[var(--text-soft)]"><div class="mx-auto max-w-sm rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface-2)] p-5">Nenhuma sala encontrada.</div></td></tr>';
        ...
        document.getElementById('resultsCount').textContent = "Erro de ligação ao servidor";
    }
}
...
async function saveRoom(e) {
    ...
    const urlPath = id ? `/rooms/${id}` : '/rooms';
