...
|--------------------------------------------------------------------------
|
| Botão otimizado para autenticação e login social (OAuth).
| • Inclui ícones e rótulos oficiais integrados do Design System.
| • Suporte a slots para personalizar tanto o ícone como o texto.
| • Proteção nativa contra vulnerabilidades de redirecionamento (Tabnabbing).
|
--}}
...
@php
    ...
    // SVGs Oficiais dos fornecedores
    $defaultIcons = [
    ...
    if ($tag === 'button') {
        ...
        if ($disabled) {
            ...
            if (! $attributes->has('rel')) {
                $customAttributes['rel'] = 'noopener noreferrer';
            }
        }
    }

    // Seleção de ícone: slot dedicado > prop icon > SVG padrão oficial
    $selectedIcon = $iconSlot ?? $icon ?? ($defaultIcons[$providerKey] ?? null);

    // Seleção do texto: prop label > slot principal > rótulo pré-definido > nome da provider
    $displayText = $label ?? ($slot->isNotEmpty() ? $slot : ($providerLabels[$providerKey] ?? ucfirst($provider)));
@endphp

<{{ $tag }}
    {{ $attributes->merge($customAttributes)->class([
        'ui-social-button',
        "ui-social-button--{$providerKey}",
...
>
    ...
    @if($selectedIcon)
        <span class="ui-social-button__icon" aria-hidden="true">
            {{ is_string($selectedIcon) ? {!! $selectedIcon !!} : $selectedIcon }}
        </span>
    @endif
