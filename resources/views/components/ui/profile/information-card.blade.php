...
<x-ui.form.card
    :title="__('Informações Pessoais')"
    :description="__('Atualize os seus dados pessoais.')"
...
>
    <form
        ...
        class="space-y-4"
        novalidate
        data-validation-message="{{ __('Introduza um nome para continuar.') }}"
        data-saving-message="{{ __('A guardar alterações...') }}"
        data-success-message="{{ __('Perfil atualizado com sucesso.') }}"
        data-error-message="{{ __('Não foi possível atualizar o perfil.') }}"
    >
        ...
        <x-ui.form.field :id="'profileEmail'" :label="__('Email')">
            <x-ui.form.input
                id="profileEmail"
                name="email"
                type="email"
        ...
        <p class="text-xs text-[var(--text-soft)]">{{ __('O email é gerido pela administração e não pode ser alterado.') }}</p>
